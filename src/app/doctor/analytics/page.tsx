...
const visitData = [
  ...
  { name: 'Consult', online: 20, walkin: 12 },
...
const TT = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '10px 14px', boxShadow: '0 4px 16px rgba(0,0,0,.08)' }}>
      <p style={{ fontSize: 12, fontWeight: 700, color: '#0F172A', marginBottom: 4, borderBottom: '1px solid #E2E8F0', paddingBottom: 4 }}>{label}</p>
      {payload.map((e: any, i: number) => (
        <p key={i} style={{ fontSize: 12, fontWeight: 600, color: e.color }}>
          {e.name === 'revenue' ? `₹${(e.value / 1000).toFixed(1)}k` : e.value} <span style={{ color: '#94A3B8', fontWeight: 400 }}>{e.name}</span>
        </p>
      ))}
    </div>
  )
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {[
          { label: 'Patients This Week', v: '112', sub: '+8 vs last week', icon: Users, col: 'text-primary', bg: 'bg-primary-light' },
          ...
          { label: 'Avg Consult Time', v: '14 min', sub: 'Down 2 min vs last week', icon: Clock, col: 'text-accent', bg: 'bg-accent-light' },
        ].map(stat => (
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={weeklyPatients} margin={{ top: 5, right: 8, left: 0, bottom: 0 }}>
              <defs>
              ...
              <Area type="monotone" dataKey="patients" name="patients" stroke="#3B82F6" strokeWidth={2.5} fill="url(#gradPat)" dot={{ fill: '#3B82F6', r: 3, strokeWidth: 0 }} activeDot={{ r: 5 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={visitData} margin={{ top: 5, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="card p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={monthlyTrend} margin={{ top: 5, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis yAxisId="l" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#94A3B8' }} width={30} />
              <YAxis yAxisId="r" orientation="right" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#94A3B8' }} width={44} tickFormatter={v => `₹${v / 1000}k`} />
              <Tooltip content={<TT />} cursor={{ stroke: '#E2E8F0', strokeDasharray: '4 4' }} />
              <Line yAxisId="l" type="monotone" dataKey="patients" name="patients" stroke="#3B82F6" strokeWidth={2.5} dot={{ r: 3, fill: '#3B82F6', strokeWidth: 0 }} activeDot={{ r: 5 }} />
              <Line yAxisId="r" type="monotone" dataKey="revenue" name="revenue" stroke="#10B981" strokeWidth={2.5} dot={{ r: 3, fill: '#10B981', strokeWidth: 0 }} activeDot={{ r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={170}>
            <PieChart>
              <Pie data={visitTypes} cx="50%" cy="50%" innerRadius={44} outerRadius={76} paddingAngle={3} dataKey="value" labelLine={false}>
