...
const DOCTOR_LEAVES = [9, 16, 23] // days off (e.g. certain Sundays/Wednesdays)
...
export default function SelectDatePage() {
  ...
  const isLeave = (day: number) => DOCTOR_LEAVES.includes(day)
  const isUnavailable = (day: number) => isPast(day) || isSunday(day) || isLeave(day)
  ...
  const handleNext = () => {
    if (!selected) return
    sessionStorage.setItem('md_date', JSON.stringify({ year, month, day: selected }))
      ...
      <div className="card p-6 shadow-md">
        ...
        <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-brand-border">
          {[
            ...
            { dot: 'bg-brand-border', label: 'Unavailable / Holiday' },
        ...
        <div className="flex items-start gap-2 bg-primary-light rounded-lg px-3 py-2.5 mt-4">
          ...
          <p className="text-xs text-primary">
            Dr. Sharma is available Mon–Sat, 9:00 AM – 8:00 PM. Sundays marked are closed.
          </p>
        </div>
